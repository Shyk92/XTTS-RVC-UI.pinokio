...
# Choose devices based on availability
tts_device = "cuda:0" if cuda_available else "cpu"
rvc_device = "mps" if mps_available else tts_device	 # Use MPS for RVC if available, otherwise fallback to the TTS device

print(f"TTS Device: {tts_device}")
print(f"RVC Device: {rvc_device}")

config = Config(rvc_device, rvc_device != 'cpu')
hubert_model = load_hubert(rvc_device, config.is_half, "./models/hubert_base.pt").to(rvc_device)
tts = TTS(model_path="./models/xtts", config_path='./models/xtts/config.json').to(tts_device)
voices = []
rvcs = []
...
def main():
	...
	interface.launch(server_name="0.0.0.0", server_port=7890, quiet=True)
